...
from .models import (
    Sucursal, Empleado, Cliente, Categoria, Producto, Venta
)
...
# =========================
# VENTAS (CRUD) - UNA VENTA CON UN SOLO PRODUCTO
# =========================
def ver_ventas(request):
    ventas = Venta.objects.select_related('id_cliente', 'id_empleado', 'id_sucursal', 'id_producto').all().order_by('-fecha')
...
def agregar_venta(request):
    ...
    if request.method == 'POST':
        ...
        subtotal_raw = request.POST.get('subtotal') or '0'
        impuesto_raw = request.POST.get('impuesto') or '0'
        total_raw = request.POST.get('total') or '0'
        ...
        venta = Venta.objects.create(
            id_cliente=Cliente.objects.get(pk=id_cliente) if id_cliente else None,
            id_empleado=Empleado.objects.get(pk=id_empleado) if id_empleado else None,
            id_sucursal=Sucursal.objects.get(pk=id_sucursal) if id_sucursal else None,
            id_producto=Producto.objects.get(pk=id_producto) if id_producto else None,
...
def realizar_actualizacion_venta(request, id_venta):
    if request.method == 'POST':
        ...
        venta.id_cliente = Cliente.objects.get(pk=request.POST.get('id_cliente')) if request.POST.get('id_cliente') else None
        venta.id_empleado = Empleado.objects.get(pk=request.POST.get('id_empleado')) if request.POST.get('id_empleado') else None
        venta.id_sucursal = Sucursal.objects.get(pk=request.POST.get('id_sucursal')) if request.POST.get('id_sucursal') else None
        venta.id_producto = Producto.objects.get(pk=request.POST.get('id_producto')) if request.POST.get('id_producto') else None

        venta.metodo_pago = request.POST.get('metodo_pago', venta.metodo_pago)

        try:
            venta.subtotal = Decimal(request.POST.get('subtotal')) if request.POST.get('subtotal') not in (None, '') else venta.subtotal
        except (InvalidOperation, TypeError):
            pass
        try:
            venta.impuesto = Decimal(request.POST.get('impuesto')) if request.POST.get('impuesto') not in (None, '') else venta.impuesto
        except (InvalidOperation, TypeError):
            pass
        try:
            venta.total = Decimal(request.POST.get('total')) if request.POST.get('total') not in (None, '') else venta.total
        except (InvalidOperation, TypeError):
            pass
